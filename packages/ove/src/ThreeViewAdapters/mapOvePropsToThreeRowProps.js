function getAnnotationHandler(props, annotationType) {
  const handlerMap = {
    ...
    translation: props.translationClicked,
    orf: props.orfClicked
  };
  return handlerMap[annotationType];
}

function getAnnotationDoubleClickHandler(props, annotationType) {
  const handlerMap = {
    cutsite: props.cutsiteDoubleClicked,
    translation: props.translationDoubleClicked
  };
  return (
    handlerMap[annotationType] || getAnnotationHandler(props, annotationType)
  );
}
...
function getAnnotationRightClickHandler(props, annotationType) {
  const handlerMap = {
    ...
    translation: props.translationRightClicked,
...
function routeAnnotationDoubleClickEvent(props, annotation, userData, event) {
  const handler = getAnnotationDoubleClickHandler(
    props,
...
export default function mapOvePropsToThreeRowProps(props = {}) {
  ...
  return {
    ...
    onDoubleClickRange: (annotation, payload) => {
      routeAnnotationDoubleClickEvent(
        props,
        annotation,
        payload?.userData,
    ...
    onSelectionChange: selection => {
      props.selectionLayerUpdate?.({ ...selection, isFromRowView: true });
    }
  };
}
